...
The bootstrap kernel artifact supplies the verified TB321FU base configuration. By default, the workflow fetches the exact matching public source commit from `GUF296/linux`, enables `CONFIG_SECURITY_APPARMOR=y`, the required SquashFS decompressors, Android Binder IPC and BinderFS, memfd, namespaces, cgroups, PSI, bridge/veth networking, and the built-in legacy iptables/XFRM stack required by both the Waydroid host and Android 13 `netd`. This includes the IPv4 filter/NAT/mangle/raw and IPv6 filter/mangle/raw tables plus the NFLOG, mark/connmark, BPF, owner, policy, socket, state, limit, reject, and TCPMSS extensions used by Android networking. To leave room for those built-ins, the default build disables `CONFIG_KALLSYMS_ALL` while retaining `CONFIG_KALLSYMS=y`. The build fails if the ARM64 header declares a load size of 56 MiB or more, preventing an Image that crosses the observed QCOMRAMP direct-boot boundary from being packaged. The workflow then rebuilds `Image` plus the TB321FU DTB. GRUB is packaged with this rebuilt kernel instead of the bootstrap binary. The kernel archive, normalized configuration, build metadata, and checksums are included in the Actions artifact.
